    ...
    if (username === store.getState().username) {
      ...
      fetch(url)
        ...
        .then((response) => {
          ...
          // get user's posts
          fetch(
            `${process.env.REACT_APP_API_URL}/posts?username=${username}&onlyUserPosts=true&token=${store.getState().token}`
          )
            .then((response) => response.json())
            .then((response) => {
              console.log(response)
        ...
        <div className="profile__posts">
          ...
          <div className="profile__posts__content">
            {posts.map((post, index) =>
              <img src={post.photo} key={index} className="profile__posts__content__item" onClick={() => history.push(`/post/${post.id}`)}/>
            )}
          </div>
        </div>
      </div>
